...
public class LearningService {
    ...
    static Map<String,Object> localPlan(Map<String,Object> s){List<Object> domains=Json.arr(s.get("domains")),priorities=new ArrayList<Object>(),plan=new ArrayList<Object>();int minutes=Json.integer(Json.obj(s.get("profile")).get("daily_minutes"),60);
        ...
        String[] foundationTasks={"复习第1课的双科要求，进入第2课：创建文件夹、区分数据文件和代码文件，练习不足可顺延。","继续第2课：制作两行数据表并导出CSV，检查文件名、列名和保存位置。","进入第3课：认识Python变量与类型，独立修改并运行一个输入输出示例。","继续第3课：练习条件判断和循环，说明每一步的结果并保存代码。","进入第4课：练习列表和函数，把重复操作整理成可复用函数。","继续第4课：读写小数据表并保留运行结果，遇到错误先记录报错和解决过程。","复盘本周代码与课程自测；通过前四课后进入数据与业务阶段，未掌握的内容继续巩固。"};
